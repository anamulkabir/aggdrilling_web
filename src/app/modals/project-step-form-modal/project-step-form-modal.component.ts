...
@Component({
  selector: 'app-project-step-form-modal',
  templateUrl: './project-step-form-modal.component.html',
  styleUrls: ['./project-step-form-modal.component.scss']
})
export class ProjectStepFormModalComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog,private toastrService: ToastrService,private formBuilder: FormBuilder,private firestore:AngularFirestore,
    public dialogRef: MatDialogRef<ProjectStepFormModalComponent>,@Inject(MAT_DIALOG_DATA) public data: any) { }
  ...
  if (this.registerForm.value.id==null) {
        this.firestore.collection('projectSteps/').add(body).then(()=>{
      ...
      this.firestore.collection('projectSteps/').doc(this.registerForm.value.id).update(body).then(()=>{
